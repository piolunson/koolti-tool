...
def logo():
    ...
    print()
    cprint(f"{DIM}{'▰'*55}{RST}")
    cprint(f"  {BOLD}{LWHT}v10.0.0{RST}  {DIM}·{RST}  {LCYN}Real Engine{RST}  {DIM}·{RST}  {LMAG}151 Modules{RST}  {DIM}·{RST}  {LYLW}{datetime.now().strftime('%Y-%m-%d  %H:%M:%S')}{RST}  ")
    cprint(f"{DIM}{'▰'*55}{RST}")
    print()
...
def draw_main_menu():
    w = min(term_width(), 70)
    hline("═", LCYN, w)
    cprint(f"{BOLD}{LCYN}MAIN MENU{RST}")
    hline("═", LCYN, w)
    print()
    print(f"  {LYLW}{BOLD}[1]{RST}  🌐  {LWHT}Modules{RST}         {DIM}— browse 151 modules by category{RST}")
    print(f"  {LYLW}{BOLD}[2]{RST}  📦  {LWHT}Bundles{RST}         {DIM}— run full workflow on one target{RST}")
    print(f"  {LYLW}{BOLD}[3]{RST}  🔧  {LWHT}Tool Installer{RST}  {DIM}— install nmap, sqlmap, hashcat...{RST}")
    if _loaded_plugins:
        print(f"  {LYLW}{BOLD}[4]{RST}  🧩  {LWHT}Plugins{RST}         {DIM}— {len(_loaded_plugins)} loaded{RST}")
    print()
    hline("─", DIM, w)
    print(f"  {DIM}[h]{RST} Help   {DIM}[/]{RST} Search   {DIM}[fav]{RST} Favourites   {DIM}[u]{RST} Update   {DIM}[q]{RST} Quit")


def draw_category_menu():
    w = min(term_width(), 70)
    hline("═", LCYN, w)
    cprint(f"{BOLD}{LCYN}MODULES — SELECT CATEGORY{RST}")
    hline("═", LCYN, w)
    print()
    cat_counts = {}
    for nr, name, cat, is_new in MENU_DATA:
        cat_counts[cat] = cat_counts.get(cat, 0) + 1
    cat_order = [("NET","🌐",LBLU),("WEB","🕸",LMAG),("CRY","🔐",LYLW),("SYS","💻",LGRN),("UTL","🛠",LCYN)]
    for i, (cat, icon, col) in enumerate(cat_order, 1):
        cat_name = CATEGORIES[cat][0]
        count    = cat_counts.get(cat, 0)
        print(f"  {col}{BOLD}[{i}]{RST}  {icon}  {LWHT}{cat_name:<18}{RST}  {DIM}{count} modules{RST}")
    print()
    hline("─", DIM, w)
    print(f"  {DIM}[0]{RST} Back   {DIM}[/]{RST} Search")


def draw_module_list(cat_key):
    cat_name, cat_color, _ = CATEGORIES[cat_key]
    w = min(term_width(), 80)
    hline("═", cat_color, w)
    cprint(f"{cat_color}{BOLD}◈  {cat_name}{RST}")
    hline("═", cat_color, w)
    print()
    for nr, name, cat, is_new in MENU_DATA:
        if cat != cat_key:
        ...
        print(f"  {cat_color}{BOLD}[{nr:03d}]{RST}  {LWHT}{name}{RST}{new_tag}")
    print()
    hline("─", DIM, w)
    print(f"  {DIM}[0]{RST} Back   {DIM}Enter number to run{RST}")
...
ACTIONS = {
    ...
    151:lambda: [mod_header("IP TO ASN","Find ASN for IP","🌐",LBLU),
                 [print(f"  {LCYN}{k}{RST}  {LWHT}{v}{RST}") for k,v in [
                   ("IP",ip:=ask("IP")),
                   ("ASN",requests.get(f"https://api.bgpview.io/ip/{ip}",timeout=8).json().get("data",{}).get("prefixes",[{}])[0].get("asn",{}).get("asn","N/A") if ip else "N/A")
                 ]] if ip else None][-1],
}
...
def show_help():
    ...
    w = min(term_width(), 70)
    hline("═", LCYN, w)
    cprint(f"{BOLD}{LCYN}◈  KOOLTI-TOOL v10.0.0  —  HELP  ◈{RST}")
    hline("═", LCYN, w)
    print(f"""
  {BOLD}Main menu:{RST}
    {LYLW}[1]{RST}  Modules     — pick category, then module number
    {LYLW}[2]{RST}  Bundles     — full auto-workflow on one target
    {LYLW}[3]{RST}  Installer   — install nmap, sqlmap, hashcat etc.

  {BOLD}Shortcuts (anywhere):{RST}
  ...
  {BOLD}Bundles available:{RST}
    {DIM}Website, Hash, Email, Network Scan,
    Subdomain, WordPress, API Security, Password Audit{RST}
...
def _run_module(n):
    if n in ACTIONS:
        try:
            ACTIONS[n]()
            history_log(n, next((nm for nr,nm,*_ in MENU_DATA if nr==n), str(n)), {})
...
def _modules_menu():
    CAT_ORDER = [("NET","🌐",LBLU),("WEB","🕸",LMAG),("CRY","🔐",LYLW),("SYS","💻",LGRN),("UTL","🛠",LCYN)]
    while True:
        logo(); draw_category_menu(); print()
        cmd = input_with_history(f"  {LGRN}{BOLD}❯{RST} Category: ").strip().lower()
        if cmd in ("0","back",""): return
        if cmd in ("/","search"):  mod_search(); pause(); continue
        try:
            idx     = int(cmd) - 1
            cat_key = CAT_ORDER[idx][0]
        except (ValueError, IndexError):
            status_err("Invalid — enter 1-5"); time.sleep(0.7); continue

        while True:
            logo(); draw_module_list(cat_key); print()
            cmd2 = input_with_history(f"  {LGRN}{BOLD}❯{RST} Module #: ").strip().lower()
            if cmd2 in ("0","back",""): break
            if cmd2 in ("/","search"):  mod_search(); pause(); continue
            try:
                _run_module(int(cmd2))
            except ValueError:
                pass
...
def _plugins_menu():
    while True:
        logo()
        w = min(term_width(), 70)
        hline("═", LCYN, w); cprint(f"{BOLD}{LCYN}PLUGINS{RST}"); hline("═", LCYN, w)
        print()
        if not _loaded_plugins:
            status_warn("No plugins loaded")
            status_info(f"Drop .py files into: {Path.home() / 'kooltitool' / 'plugins'}")
        else:
            print(f"  {BOLD}{LCYN}{'SLOT':<6}{'NAME':<28}{'VER':<8}AUTHOR{RST}")
            hline("─", DIM, 55)
            for slot, p in _loaded_plugins.items():
                print(f"  {LYLW}{slot:<6}{RST}{LWHT}{p.get('name','?')[:26]:<28}{RST}"
                      f"{DIM}{p.get('version','?'):<8}{RST}{LGRN}{p.get('author','?')}{RST}")
        print(); hline("─", DIM, w)
        print(f"  {DIM}[0]{RST} Back   {DIM}Enter slot to run{RST}")
        print()
        cmd = input_with_history(f"  {LGRN}{BOLD}❯{RST} ").strip().lower()
        if cmd in ("0","back",""): return
        try:
            _run_module(int(cmd))
        except ValueError: pass
...
def main():
    ...
    while True:
        ...
        cmd = input_with_history(
            f"  {LGRN}{BOLD}❯{RST}  {DIM}(1=modules  2=bundles  3=installer  /=search  h=help){RST}\n"
            f"  {LGRN}{BOLD}❯{RST} "
        ).strip().lower()

        if cmd in ("q","0","exit","quit"):
            clear(); print(f"\n  {LGRN}Goodbye!{RST}\n"); sys.exit(0)
        elif cmd == "1":                              _modules_menu()
        elif cmd in ("2","b","bundles"):              _bundles_menu()
        elif cmd in ("3","i","installer"):            mod_tool_installer()
        elif cmd == "4" and _loaded_plugins:          _plugins_menu()
